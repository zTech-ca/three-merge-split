...
export class ThreeUtils {
  ...
  private identity: THREE.Matrix4;

  constructor({ Vector3, Matrix4 }: ThreeUtilsUtils) {
    this.v = new Vector3();
  ...
  public centerGeometryAndShiftMesh(mesh: THREE.Mesh) {
  ...
  public getAncestralTransformations(
  ...
  public getSortedIntegers(length: number, start = 0) {
    if (start) {
      let arr = [];
  ...
  public getFirstMaterial(mesh: THREE.Mesh) {
  ...
  public cloneBufferGeometry(geometry: THREE.BufferGeometry) {
  ...
  private getAncestralTransformationsRecursively(
    object: THREE.Object3D,
    startingUuid: string,
    transformations: THREE.Matrix4[] = []
  ): THREE.Matrix4[] {
  ...
  private isIdentity(matrix: THREE.Matrix4) {
